...
install -m 0600 "$TMP/config.dr1.json" /etc/hashburst/config.json
install -o synapta -g synapta -m 0755 "$MASTER" /opt/hashburst/master_node.py
...
for pair in 'monero-k325t-mainnet.service:/etc/monero-k325t/mainnet.conf' 'monero-k325t-testnet.service:/etc/monero-k325t/testnet.conf'; do
  ...
  if [[ -n "$d" ]]; then install -d -o "$u" -g "${g:-$u}" -m 0750 "$d"; fi
  if [[ -n "$l" ]]; then install -d -o "$u" -g "${g:-$u}" -m 0750 "$(dirname "$l")"; fi
done
