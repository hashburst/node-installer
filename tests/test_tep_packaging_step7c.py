...
RUNTIME = ROOT / 'tep' / 'hb_tep_runtime.py'
...
class Step7CPackagingTests(unittest.TestCase):
    def test_canonical_daemon_exists_and_compiles(self):
        ...
        self.assertTrue(RUNTIME.is_file()); ast.parse(RUNTIME.read_text())
    ...
    def test_systemd_service_is_hardened_and_uses_v215_runtime(self):
        s = SERVICE.read_text()
        for token in ['WorkingDirectory=/opt/hashburst-tep', 'ExecStart=/usr/bin/python3 -m tep.hb_tep_runtime', 'EnvironmentFile=-/etc/hashburst/hashburst-tep.env', 'NoNewPrivileges=true', 'ProtectSystem=strict', 'ReadWritePaths=/var/lib/hashburst /var/log/hashburst']:
            self.assertIn(token, s)
        for token in ['8093', '8094', '8095', '5011']:
            self.assertNotIn(token, s)

    def test_installer_packages_tep_and_uses_onboard_helper(self):
        s = INSTALLER.read_text()
        self.assertIn('/opt/hashburst-tep/tep', s)
        self.assertIn('config/hashburst-tep.env.example', s)
        self.assertIn('bin/hb-tep-onboard', s)
        self.assertIn('bash "$SCRIPT_DIR/bin/hb-tep-onboard"', s)
